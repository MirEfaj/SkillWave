...
class _VideoPageState extends State<VideoPage> with WidgetsBindingObserver {
  ...
  bool _isLandscape = false; // Track fullscreen/landscape
  ...
  void dispose() {
    WidgetsBinding.instance.removeObserver(this);
    _ytController.dispose();
    _exitFullScreen(); // Reset orientation
    super.dispose();
  }
  ...
  Future<void> _loadEnrollmentAndProgress() async {
    final enrolled = await CourseProgressService.isEnrolled(widget.courseId, 'video');
    final progress = await CourseProgressService.getProgress(widget.courseId, 'video');
    if (!mounted) return;
    setState(() {
      _enrolled = enrolled;
      _progress = progress;
  ...
  Future<void> _toggleEnrollment() async {
    if (_enrolled) {
      await CourseProgressService.unenroll(widget.courseId, 'video', resetProgress: true);
      setState(() {
        _enrolled = false;
        _progress = 0.0;
      });
  ...
  Future<void> _markLessonComplete() async {
    ...
    final total = widget.videoUrls.isEmpty ? 1 : widget.videoUrls.length;
    final step = 1.0 / total;
    await CourseProgressService.incrementProgressBy(widget.courseId, 'video', step);
    final newProgress = await CourseProgressService.getProgress(widget.courseId, 'video');
    if (!mounted) return;
    setState(() => _progress = newProgress);

    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Lesson completed — progress ${(newProgress * 100).toStringAsFixed(0)}%')),
    );
  }
